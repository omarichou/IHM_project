...
def edit_category(cat_id):
    ...
    try:
        ...
        db.session.rollback()
        app.logger.error(f"Erreur lors de la mise à jour de la catégorie: {str(e)}")
...
def delete_category(cat_id):
    cat = Category.query.get_or_404(cat_id)
    try:
        # If dishes reference this category, clear association first
        Dish.query.filter_by(category_id=cat.id).update({'category_id': None})
        db.session.delete(cat)
        db.session.commit()
        flash('Catégorie supprimée avec succès.', 'success')
    except Exception as e:
        db.session.rollback()
        app.logger.error(f"Erreur lors de la suppression de la catégorie: {str(e)}")
        flash('Impossible de supprimer la catégorie.', 'danger')
    return redirect(url_for('admin_categories'))
...
def api_category(cat_id):
    """JSON API for updating or deleting a category (used by admin UI JS)."""
    cat = Category.query.get_or_404(cat_id)
    if request.method == 'PUT':
        ...
        try:
            ...
            db.session.rollback()
            app.logger.error(f"API update category error: {e}")
    ...
    if request.method == 'DELETE':
        try:
            ...
            db.session.rollback()
            app.logger.error(f"API delete category error: {e}")
...
def admin_dishes():
    ...
    return render_template('admin_dishes.html', form=form, dishes=dishes, categories=categories)


@app.route('/admin/api/dishes/<int:d_id>', methods=['PUT', 'DELETE'])
@admin_required
def api_dish(d_id):
    d = Dish.query.get_or_404(d_id)
    if request.method == 'PUT':
        data = request.get_json() or {}
        name = data.get('name')
        ...
        try:
            ...
            d.price = float(price)
            d.cost = float(cost) if cost is not None else (d.cost or 0)
            # handle category id or name
            if category is None:
                d.category = None
            else:
                if isinstance(category, int) or (isinstance(category, str) and str(category).isdigit()):
                    cat = Category.query.get(int(category))
                else:
                    cat = Category.query.filter_by(name=category).first()
                if cat is None and isinstance(category, str):
                    cat = Category(name=category)
                    db.session.add(cat)
                    db.session.flush()
                d.category = cat
            db.session.commit()
            res = { 'id': d.id, 'name': d.name, 'description': d.description, 'price': d.price, 'cost': d.cost }
            if d.category:
                res['category'] = { 'id': d.category.id, 'name': d.category.name }
            else:
                res['category'] = None
            return jsonify(res), 200
        except Exception as e:
            db.session.rollback()
            app.logger.error(f"API update dish error: {e}")
            return jsonify({'error': 'update failed'}), 500
    ...
    if request.method == 'DELETE':
        try:
            ...
            db.session.rollback()
            app.logger.error(f"API delete dish error: {e}")
            return jsonify({'error': 'delete failed'}), 500
